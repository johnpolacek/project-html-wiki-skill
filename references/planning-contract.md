...
- `record_execution`: append durable project-context history to `wiki/log.html` after implementation, validation, or maintenance work changes plans, roadmap, source context, or important decisions.
...
Do not create durable implementation plans in root-level `docs/`, root-level `tasks/`, issue-only notes, or unindexed scratch files. Keep them under `wiki/plans/` and update `wiki/index.html` if the new page is a durable wiki entry.
...
Plan files must be useful from a terminal. Optimize the first 80 lines of `wiki/plans/index.html` and every active plan for quick inspection with `sed`, `head`, or `less`.

`wiki/plans/index.html` is the plan dashboard. Keep its top section current enough to answer:
...
Completed plans should not remain in active current-plan or current-unit slots. When a whole plan is complete, move it under `wiki/plans/zzz_completed/` and keep only a compact completed-plan link in `wiki/plans/index.html`.

Every active plan should start with a compact summary section:

```html
<section class="summary" aria-labelledby="summary">
  <h2 id="summary">Summary</h2>
  <dl>
    <dt>Status</dt><dd>draft|active|blocked|complete|deferred</dd>
    <dt>Shape</dt><dd>compact feature plan|single-stage MVP|multi-stage MVP</dd>
    <dt>Current unit</dt><dd>[unit name or none]</dd>
    <dt>Next action</dt><dd>[one concrete action or decision]</dd>
    <dt>Blockers</dt><dd>[none or short list]</dd>
    <dt>Validation</dt><dd>[key checks]</dd>
  </dl>
</section>
```
...
1. Read `wiki/plans/index.html` first.
...
4. Summarize current state in the response instead of dumping long HTML.
5. Provide exact paths and useful read commands, such as `sed -n '1,80p' wiki/plans/index.html`.
...
Use $project-html-wiki to update the current plan from recent changes.
...
```text
Current plan: wiki/plans/features/blog-authoring.html
...
Avoid large HTML dumps unless the user explicitly asks to see the full file. Prefer a concise terminal summary plus file paths and commands.
...
- design considerations for UI work, preferably by linking to `../../sources/design-brief.html` or another relative path instead of duplicating the brief
...
- existing `wiki/index.html`, `wiki/roadmap.html`, `wiki/Sources.html`, source briefs, plans, and `wiki/log.html`
...
- Prefer appending `wiki/log.html` for material completed work, validation, decisions, and follow-ups that affect durable project context.
...
- Update `wiki/plans/mvp/index.html` or stage/unit status only when the current next unit or planning target changed.
- Update `wiki/roadmap.html` when the current goal, next decision, next steps, or deferred work changed.
- Update `wiki/Sources.html` or source briefs when code changes reveal durable product, technical, integration, runtime, data, UI design, or public-entry facts.
...
- Update `wiki/index.html` only when adding or materially changing durable wiki pages.
...
Do not create `wiki/plans/mvp/` during import for existing live products, internal tools in active use, libraries, archives, maintenance-mode projects, or unknown lifecycle. Use `wiki/roadmap.html` for broad direction and create focused plans only when a concrete workstream exists:
...
Do not create `wiki/plans/mvp/` for a very small `bootstrap_new` handoff, one-file script, research note, existing live project, or project that only needs a compact `wiki/roadmap.html` plus `wiki/plans/index.html`.
...
If a roadmap item is important but not ready to plan into multiple executable units, keep it as deferred work in `wiki/roadmap.html`, `wiki/plans/mvp/index.html`, or the current stage's follow-up section instead of creating a placeholder stage.
...
wiki/plans/
  ...
  mvp/
    index.html
    stage-01-foundation.html
    units/
      stage-01/
        01-define-stack.html
        02-create-minimal-app-shell.html
  zzz_completed/
    mvp/
      index.html
      stage-01-foundation.html
...
`wiki/plans/mvp/index.html` should act as the live roadmap index. It should name the current status, most recent completed unit when known, and exactly one next execution unit or planning target.
...
```html
<article class="unit">
  <h1>Unit 01 - Local Admin Authoring Foundation</h1>
  ...
  <section>
    ...
    <h3>Manual</h3><ul><li>Open <code>/admin</code>.</li><li>Create a draft post.</li><li>Preview the draft.</li><li>Confirm public posts are unchanged.</li></ul>
    <h3>Deferred</h3><p>None.</p>
  </section>
  <section><h2>Completion Gate</h2><p>This unit is complete when the guarded admin authoring shell can create, save, validate, and preview a draft without changing public publishing behavior.</p></section>
</article>
```
...
- `wiki/plans/features/feature-slug.html` moves to `wiki/plans/zzz_completed/features/feature-slug.html`.
...
- no current blocker or next action remains inside the plan except follow-up work that has been moved to a new active plan, roadmap deferred work, or `wiki/log.html`
...
- add or update a compact completed section in `wiki/plans/index.html` linking to the archived location
- choose the next active plan or state that no active plan exists
- append `wiki/log.html` only when completion changes durable project context, validation status, source truth, roadmap direction, or future planning

## `wiki/log.html` As Project Context History

Keep plans focused on intent. Keep routine implementation history in Git. Use `wiki/log.html` only for durable project-context history.
...
Do not backfill long narrative history into unit plans unless the plan's scope or contract materially changed. If `wiki/log.html` becomes hard to scan, summarize older entries into `wiki/log-archive.html` or a dated archive such as `wiki/log/2026.html`, and leave a pointer from the current log.
